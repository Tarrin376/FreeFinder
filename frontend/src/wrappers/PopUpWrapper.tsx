    ...
    return (
        <div className="fixed top-0 left-0 w-full h-full bg-[#00000057] z-50 flex items-center justify-center">
            ...
            animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.80 }} transition={{ type: "spring", duration: 0.4 }}>
                <div className={`${windowSize >= 500 ? "p-5" : "p-4"} border-b border-light-border-gray w-full relative`}>
                    <div className={`absolute ${windowSize >= 500 ? "top-[23px] left-5" : "top-[19px] left-4"}`}>
                        <CloseSvg
                            size={22}
                            colour="#9c9c9c"
                            action={closePopUp}
                ...
                <div className={`overflow-y-scroll flex flex-col h-full ${windowSize >= 500 ? "p-5" : "p-4"} ${firstChildStyles}`}>
                    {Array.isArray(children) ? children[0] : children}
                </div>
                {Array.isArray(children) &&
                <div className={`${windowSize >= 500 ? "p-5" : "p-4"} border-t border-light-border-gray`}>
                    {children[1]}
                </div>}
            </motion.div>
        </div>
