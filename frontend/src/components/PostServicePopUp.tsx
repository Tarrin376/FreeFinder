...
function PostServicePopUp({ setPostService, setUserPosts, cursor, setReachedBottom, setNextPage }: PostServiceProps) {
    ...
    if (section === Sections.UploadFiles) {
        return (
            <UploadPostFiles setPostService={setPostService} setSection={setSection} />
        );
    } else {
        return (
            <PostDetails 
                ...
                about={about} setAbout={setAbout} title={title} setTitle={setTitle}
...
function UploadPostFiles({ setPostService, setSection }: UploadPostFilesProps) {
    function handleDrop() {
    ...
    return (
        <PopUpWrapper setIsOpen={setPostService} title={"Upload files"}>
            <DragAndDrop handleDrop={handleDrop}>
                <div className="w-full h-[260px] outline-2 outline-dashed outline-light-gray rounded-[8px] relative">
                    <div className="absolute top-[50%] left-[50%] translate-x-[-50%] translate-y-[-50%]">
                        <img src={Storage} className="block m-auto w-[50px] h-[50px] mb-3" alt="storage" />
                        <p className="text-center">
                            Drag and Drop file here or
                            <span className="underline font-semibold cursor-pointer"> Choose file</span>
                        </p>
                    </div>
                </div>
            </DragAndDrop>
            ...
            <div className="mt-6 px-5 py-3 rounded-[8px] bg-[#f0f2f3] flex justify-between gap-[16px] items-center">
                <div>
                    <p className="font-semibold mb-1">Table Example</p>
                    <p className="text-side-text-gray">You can download your submitted file to verify that it is the correct one.</p>
                </div>
        ...
        <PopUpWrapper setIsOpen={setPostService} title={"Enter post details"}>
            ...
            <div className="flex justify-end gap-3 mt-[35px]">
                ...
                <LoadingButton
                    ...
                    callback={createPost} styles={`w-[110px] px-3 ${!validInputs() ? "invalid-button" : "btn-primary action-btn"}`}
