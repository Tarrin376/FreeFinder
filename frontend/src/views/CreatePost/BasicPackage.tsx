    ...
    return (
        <PopUpWrapper setIsOpen={setPostService} title={"Basic package details"}>
            ...
            <ul className="items-center w-full text-sm flex bg-[#f0f2f3] rounded-[8px] px-2 mb-4">
                {new Array(MAX_REVISIONS).fill(true).map((_, index) => <RevisionListItem numRevisions={index + 1} setRevisions={setRevisions} />)}
            ...
            <p className="text-side-text-gray mb-2">Features added:
                <span className={features.length === MAX_FEATURES ? 'text-error-red' : 'text-[#36BF54]'}>
